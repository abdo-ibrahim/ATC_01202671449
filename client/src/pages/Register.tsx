  ...
  return (
    <div className="register bg-gray-50 min-h-[calc(100vh-147px)] flex items-center justify-center">
      <form action="" className="bg-white shadow-md rounded-lg p-8 max-w-lg w-full">
        ...
        <p className="text-secondary mb-6 text-center">Sign up to book amazing events</p>
        {error && <p className="text-red-500 text-center mb-4">{error}</p>}
        <div className="grid w-full max-w-lg items-center gap-1.5 mb-5">
          <label htmlFor="name">Name</label>
          <Input type="email" id="name" placeholder="ahmed mohamed" className="py-[20px]" required value={name} onChange={(e) => setName(e.target.value)} />
        </div>
        <div className="grid w-full max-w-lg items-center gap-1.5 mb-5">
        ...
        <Button type="submit" className="w-full mb-5" onClick={handleSubmit}>
          {isLoadingButton ? <div className="w-5 h-5 border-2 border-t-transparent border-white rounded-full animate-spin"></div> : "Register"}
        </Button>
